...
CAMSV_CUSTOM_BUFFER_INFO getAeBufSize_motaustingc02m1bmipiraw();
CAMSV_CUSTOM_BUFFER_INFO getFlkBufsize_motaustingc02m1bmipiraw();
UINT32 DecodeAe_motaustingc02m1bmipiraw(VOID* const pData);
UINT32 DecodeFlk_motaustingc02m1bmipiraw(VOID* const pData);
...
impCamsvBufSize(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature) const
{
    ...
    switch(CamsvCusDecodeFeature)
    {
        case CAMSV_CUSTOM_DECODE_AE:
            buf_size = getAeBufSize_motaustingc02m1bmipiraw();
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            buf_size = getFlkBufsize_motaustingc02m1bmipiraw();
            break;
        default:
...
impCamsvCustomDecode(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature, VOID* const pData) const
{
    ...
    {
        case CAMSV_CUSTOM_DECODE_AE:
            DecodeAe_motaustingc02m1bmipiraw(pData);
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            DecodeFlk_motaustingc02m1bmipiraw(pData);
            break;
        default:
...
UINT32 DecodeAe_motaustingc02m1bmipiraw(VOID* const pData)
{
    ...
    CAMSV_CUSTOM_BUFFER_INFO buf = getAeBufSize_motaustingc02m1bmipiraw();
    memset(pOutput, 0, buf.size * sizeof(MUINT32));
    MUINT32 u4AEHistTotalSum = 6209280; // Default for SAIPAN_CXT_GC02M1B
...
UINT32 DecodeFlk_motaustingc02m1bmipiraw(VOID* const pData)
{
    ...
    CAMSV_CUSTOM_BUFFER_INFO buf = getFlkBufsize_motaustingc02m1bmipiraw();
    memset(pOutput, 0, buf.size * sizeof(MUINT32));
