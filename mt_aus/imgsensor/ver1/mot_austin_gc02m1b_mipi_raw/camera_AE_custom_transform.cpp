...
//camera_info_header
#include "camera_info_motaustingc02m1bmipiraw.h"
...
UINT32 transBinSum_motaustingc02m1bmipiraw(VOID* const pData);
UINT32 transATR_motaustingc02m1bmipiraw(VOID* const pData);
...
impAECustomTransform(AE_CUSTOM_TRANSFORM_ENUM const AECusFuncType, VOID* const pData) const
{
    switch(AECusFuncType)
    {
        case AE_CUSTOM_TRANSFORM_BINSUM:
            transBinSum_motaustingc02m1bmipiraw(pData);
            break;
        case AE_CUSTOM_TRANSFORM_ATR:
            transATR_motaustingc02m1bmipiraw(pData);
            break;
        default:
...
UINT32 transBinSumToEvSetting_motaustingc02m1bmipiraw(strEvSetting &EvSetting, MUINT32 u4MaxAfeGain, MUINT32 u4Magnification)
{
...
UINT32 transBinSumToHdrEvSetting_motaustingc02m1bmipiraw(strHdrEvSetting &HdrEvSetting, MUINT32 u4MaxAfeGain, MUINT32 u4Magnification)
{
    // 1. LE
...
UINT32 transBinSum_motaustingc02m1bmipiraw(VOID* const pData)
{
    ...
    transBinSumToEvSetting_motaustingc02m1bmipiraw(pAEData->pRealSetting->EvSetting, u4MaxAfeGain, u4Magnification);
    transBinSumToHdrEvSetting_motaustingc02m1bmipiraw(pAEData->pRealSetting->HdrEvSetting, u4MaxAfeGain, u4Magnification);
