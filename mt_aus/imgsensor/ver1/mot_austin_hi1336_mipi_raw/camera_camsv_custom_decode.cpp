...
CAMSV_CUSTOM_BUFFER_INFO getAeBufSize_motaustinhi1336mipiraw();
CAMSV_CUSTOM_BUFFER_INFO getFlkBufsize_motaustinhi1336mipiraw();
UINT32 DecodeAe_motaustinhi1336mipiraw(VOID* const pData);
UINT32 DecodeFlk_motaustinhi1336mipiraw(VOID* const pData);
...
impCamsvBufSize(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature) const
{
    ...
    switch(CamsvCusDecodeFeature)
    {
        case CAMSV_CUSTOM_DECODE_AE:
            buf_size = getAeBufSize_motaustinhi1336mipiraw();
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            buf_size = getFlkBufsize_motaustinhi1336mipiraw();
            break;
        default:
...
impCamsvCustomDecode(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature, VOID* const pData) const
{
    ...
    {
        case CAMSV_CUSTOM_DECODE_AE:
            DecodeAe_motaustinhi1336mipiraw(pData);
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            DecodeFlk_motaustinhi1336mipiraw(pData);
            break;
        default:
...
UINT32 DecodeAe_motaustinhi1336mipiraw(VOID* const pData)
{
    ...
    CAMSV_CUSTOM_BUFFER_INFO buf = getAeBufSize_motaustinhi1336mipiraw();
    memset(pOutput, 0, buf.size * sizeof(MUINT32));
    MUINT32 u4AEHistTotalSum = 6209280; // Default for SAIPAN_DMEGC_HI1336
...
UINT32 DecodeFlk_motaustinhi1336mipiraw(VOID* const pData)
{
    ...
    CAMSV_CUSTOM_BUFFER_INFO buf = getFlkBufsize_motaustinhi1336mipiraw();
    memset(pOutput, 0, buf.size * sizeof(MUINT32));
